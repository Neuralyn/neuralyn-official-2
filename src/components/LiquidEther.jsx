  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      setCoords(x, y) {
        ...
        // Reset user control after 5000ms of inactivity
      ...
      onDocumentTouchStart(event) {
        if (event.touches.length === 1) {
          ...
          if (this.isAutoActive && !this.hasUserControl && !this.takeoverActive) {
            const rect = this.container.getBoundingClientRect();
            const nx = (t.pageX - rect.left) / rect.width;
            const ny = (t.pageY - rect.top) / rect.height;
            this.takeoverFrom.copy(this.coords);
            this.takeoverTo.set(nx * 2 - 1, -(ny * 2 - 1));
            this.takeoverStartTime = performance.now();
            this.takeoverActive = true;
      ...
      update() {
        ...
        // Only use auto intensity if we're in auto mode and don't have user control
        if (this.isAutoActive && !this.hasUserControl && !this.takeoverActive) this.diff.multiplyScalar(this.autoIntensity);
      }
    }
    ...
    class AutoDriver {
      ...
      update() {
        ...
        // Don't start auto demo if user has control
        if (this.mouse.hasUserControl) {
